...
# Running cmake on the CMakeLists.txt in the "lexical_analyzer" (Parent) directory.
...
#    everytime because if we build this module with -DBUILD_UNIT_TEST=ON this value will be
#    cached and if we didn't turn this option off explicity using -DBUILD_UNIT_TEST=OFF
#    cmake will use the cached value and will build the tests accordingly even if we don't
#    need them in our build.
#cmake ..
